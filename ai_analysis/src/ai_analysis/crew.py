...
class AiAnalysis:
    ...
    def data_preparer(self) -> Agent:
        return Agent(
            ...
            tools=[LoadDataTool(), DataCleanerTool(), OutlierRemoverTool()],
            verbose=True,
            memory=True,
        )
    ...
    def data_cleaning_task(self) -> Task:
        return Task(config=self.tasks_config["data_cleaning_task"])
    ...
    def visualization_task(self) -> Task:
        return Task(config=self.tasks_config["visualization_task"], output_dir="knowledge/plots")
