...
class VisualizationBaseTool(BaseTool):
    ...
    def _get_clean_data(self) -> pd.DataFrame | str:
        try:
...
class BarChartAvgPriceTool(VisualizationBaseTool):
    ...
    def _run(self, filename: str) -> str:
        ...
        try:
            ...
            plt.title(
                "Price Distribution by Category (Cleaned Data)\nwith Standard Deviation",
                pad=20,
            )
            ...
            return f"Chart creation failed: {e!s}"
        finally:
            plt.close("all")
...
class ScatterPlotTool(VisualizationBaseTool):
    ...
    def _run(self, filename: str) -> str:
        ...
        try:
            ...
            return f"Scatter plot failed: {e!s}"
        finally:
            plt.close()
...
class BarChartTopRatedBrandsTool(VisualizationBaseTool):
    ...
    def _run(self, top_n: int, filename: str) -> str:
        ...
        try:
            ...
            result = data.groupby("Brand")["Rating"].mean().sort_values(ascending=False).head(top_n).reset_index()
            ...
            return f"Failed to generate chart: {e!s}"
        finally:
            plt.close("all")
